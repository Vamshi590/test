...
questions.get("/:id", async (c) => {
  ...
  try {
    ...
    const sortedQuestions = allQuestions.sort((a, b) => {
      ...
      scoreA += urgencyScore[a.urgency as keyof typeof urgencyScore] || 0;
      scoreB += urgencyScore[b.urgency as keyof typeof urgencyScore] || 0;

      // Score based on user attributes
      if (a.User?.city === user.city && a.User?.department === user.department) scoreA += 20;
      if (b.User?.city === user.city && b.User?.department === user.department) scoreB += 20;

      if (a.User?.organisation_name === user.organisation_name && a.User?.department === user.department) scoreA += 15;
      if (b.User?.organisation_name === user.organisation_name && b.User?.department === user.department) scoreB += 15;
      ...
      if (userSocieties.size > 0) {
        if (a.User?.memberships?.some(m => userSocieties.has(m.societyname))) scoreA += 10;
        if (b.User?.memberships?.some(m => userSocieties.has(m.societyname))) scoreB += 10;
      }
