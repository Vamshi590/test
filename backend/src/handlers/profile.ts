...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    ...
    switch (path) {
      ...
      case "/add-achievements-awards/{id}":
        ...
        if (awardid) {
          const uachievement = await prisma.achievementsAwards.update({
            where: { id: awardid },
            data: {
              awardName: body.awardName,
              awardedBy: body.awardedBy,
              awardedOn: body.awardDate,
              descreption: body.descreption,
              awardMedia: body.awardMediaLink,
            },
          });
          return createResponse(200, uachievement);

        }
        const achievement = await prisma.achievementsAwards.create({
          data: {
            userId: id,
            awardName: body.awardName,
            awardedBy: body.awardedBy,
        ...
        return createResponse(200, achievement);
