...
feed.get("/:id", async (c) => {
    ...
    try {
        ...
        const calculateScore = (item) => {
            ...
            if (itemUser?.city === user.city)
                score += 20;
            if (itemUser?.department === user.department)
                score += 30;
            // Organization match
            if (itemUser?.organisation_name === user.organisation_name)
                score += 25;
            // Membership match
            const userSocieties = new Set(user.memberships
                .map(m => m.societyname)
                .filter((name) => name !== null));
            if (userSocieties.size > 0 && itemUser?.memberships?.some((m) => userSocieties.has(m.societyname)))
                score += 15;
            // Urgency score for questions
            if ('urgency' in item) {
                const urgencyScore = { HIGH: 15, MEDIUM: 10, LOW: 5 };
                score += urgencyScore[item.urgency] || 0;
            }
